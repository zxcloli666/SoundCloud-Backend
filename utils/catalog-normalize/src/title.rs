...
use crate::names::{compact_key, fold_chars, same_artist, split_artist_list};
...
pub fn parse_sc_title(raw: &str, uploader: Option<&str>) -> ParsedTitle {
    ...
    if artist_part.is_none()
        && let Some(c) = RE_QUOTED_TITLE.captures(&stripped)
    {
    ...
    if artist_part.is_none()
        && let Some((l, r)) = split_by_keyword(&stripped)
