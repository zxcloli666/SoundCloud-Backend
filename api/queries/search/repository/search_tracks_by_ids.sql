...
WHERE sharing = 'public'
  ...
  AND ($1::text IS NULL OR
   ...
   OR (NOT $8::bool AND (LOWER (title) LIKE $1
   OR LOWER (uploader_username) LIKE $1))
    )
  AND sc_track_id = ANY($5::text[])
  AND ($6::text[] IS NULL OR LOWER(genre) = ANY(ARRAY(SELECT LOWER(value) FROM unnest($6::text[]) value)))
  AND ($7::text[] IS NULL OR tags && $7)
ORDER BY array_position($5::text[], sc_track_id), play_count_sc DESC NULLS LAST, sc_synced_at DESC, id DESC
    LIMIT $2
OFFSET $3
