...
use catalog_normalize::{name_in, normalize_name};
use catalog_sources::{MbArtist, MbRecording};

use super::signals::LocalSignals;
use super::{
    AlbumCandidate, ArtistCandidate, CreditEvidence, ResolveResult, ResolveSource, ResolverDeps,
    TrackContext,
};
...
pub(super) async fn search(
        ...
        && let Some(mb_primary) = rec.primary_artist.as_ref()
    {
        let local: Vec<&str> = signals
            ...
            .collect();
        if !name_in(&mb_primary.name, local.iter().copied()) {
...
pub(super) fn from_mb(
    ...
    let map_artist = |a: MbArtist, sc: Option<String>| {
        ArtistCandidate {
            name: a.name,
            mb_id: Some(a.mb_id),
            genius_id: None,
