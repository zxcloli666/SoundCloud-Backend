...
use catalog_sources::{GeniusArtistRef, GeniusService, GeniusSongMeta};
...
pub async fn search(
    ...
    cleaned_title: &str,
) -> EnrichResult<Option<ResolveResult>> {
...
fn into_result(
    ...
    score: f32,
    isrc: Option<String>,
    album: Option<(catalog_sources::GeniusAlbumRef, Option<i16>)>,
...
fn map_ref(a: GeniusArtistRef) -> ArtistCandidate {
    let has_external_id = a.genius_artist_id.is_some();
    ArtistCandidate {
        name: a.name,
        mb_id: None,
    ...
    .attributed(
        ResolveSource::Genius,
        if has_external_id { 0.85 } else { 0.7 },
        if has_external_id {
