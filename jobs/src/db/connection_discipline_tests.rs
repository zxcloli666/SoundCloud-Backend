...
fn no_pooled_connection_is_held_across_a_network_await() {
    ...
    assert!(
        reported.is_empty(),
        "a pooled PostgreSQL connection must never stay open across SoundCloud, relay, Redis, \
         Qdrant, worker or NATS work; release it before the call and take a new one after: \
         {reported:#?}"
    );
}
...
fn the_guard_sees_a_connection_held_across_a_network_call() {
    let offending = "    let mut tx = self.pool.begin().await?;\n\
                     let track = self.client.track(id).await?;\n\
                     tx.commit().await?;\n";
    assert_eq!(held_blocks(offending).len(), 1);

    let compliant = "    let track = self.client.track(id).await?;\n\
